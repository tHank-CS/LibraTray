...
theme, language and OSD preferences, and Windows automation switches. It does
not export an account credential, cloud token, device address, or protocol
device ID. Import accepts at most 1 MiB of valid UTF-8, validates both the
envelope version and settings schema, shows a summary, and requires an explicit
Save before applying changes. Export warns before opening the destination
picker and replaces the destination through a same-directory temporary file.
Users must inspect a configuration export before sharing it because aliases and
usage preferences may still be sensitive.
