...
> **当前状态：**v1.1.0 是最新稳定版，加入默认关闭的实验性左右分区 RGB
> 与 Windows 生命周期修复。是否已经公开发布，以项目的
> [GitHub Releases](https://github.com/tHank-CS/LibraTray/releases) 页面为准。
> 当前产物暂无代码签名。分区颜色只能表示已请求并由人眼确认的值，不是设备可读状态；
> 单台设备无法隔离冷启动故障因果关系的限制仍然存在。
...
- 仅限精确 `lamp15`、默认关闭的实验性氛围灯左右分区 RGB，包括固件门禁、请求值标记、
  分区预设、受保护的 Windows 自启回放和恢复整灯颜色入口；
- 手动触发、有界执行的氛围灯渲染器自检与恢复；
- 有界重试、通知复读和单连接命令节流；
...
- 可选的锁屏/显示器电源联动、受保护的关机/启动恢复，以及独立的登录后双灯自动开启策略；
- 精简的设备信息窗口和可选的有界脱敏诊断摘要；
- 自动化测试与模拟设备测试面。

版本信息：[v1.1.0 发布说明](docs/releases/v1.1.0.zh-CN.md)。
...
受保护的关机/启动恢复。WTS 锁定状态、安全屏保状态和显示器电源会融合为阻止条件；
事件重叠或乱序时，只有全部阻止条件解除后才恢复。屏保结束但仍停留在 Windows 锁屏
界面时不会亮灯。近期手动操作和恢复前发现的新设备状态始终优先。

启用“登录 Windows 时自动启动 LibraTray”后，还可独立启用“登录后自动打开主灯和
氛围灯”。该策略不需要关机恢复凭据：Windows `--startup` 启动会最多等待设备一分钟，
随后在保留现场亮度、色温和颜色目标的前提下打开两个通道。若同时启用条件关机恢复，程序会
先评估恢复凭据，最后执行明确的登录开灯策略；等待期间的手动操作会取消此次自动开灯。

关机恢复使用 `%LOCALAPPDATA%\LibraTray\shutdown-restore.json` 中的一次性凭据。凭据会在
Windows 首次查询结束会话时、进入确认关灯阶段之前原子保存，只包含精确设备 ID 的哈希而非
地址，七天后过期，并且只在 Windows `--startup` 启动中消费或丢弃；若关机被取消则清除
预存凭据。启动后最多等待局域网设备一分钟；仅当复读确认设备仍为预期关灯状态时才恢复，
若设备实际没有关灯则直接清除凭据。关机阶段只发送必要的电源写入，最多占用八秒，不会无限
拖住系统。
...
语言偏好；预设可包含可选的分区请求。独立运行状态只保存设备键哈希、最后请求模式/颜色和
固件风险确认，不随设置导出。配置损坏或版本不受支持时会恢复安全默认值，不会保存账号、云端 Token 或
设备地址。
