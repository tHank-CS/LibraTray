...
> Windows 自动化、打包、签名和 GitHub Release 仍待完成。氛围灯左右分区 RGB
> 命令虽能即时生效，但它与已观察到的
...
生命周期自动化将逐项启用并进行防抖。普通界面不会把 `lamp15` 显示成设备名称。

设置保存在 `%LOCALAPPDATA%\LibraTray\settings.json`。当前版本只保存设备别名、调节
步进、快捷键绑定、托盘滚轮偏好和最多 20 个本地预设。配置损坏或版本不受支持时会
恢复安全默认值，不会保存账号、云端 Token 或设备地址。
