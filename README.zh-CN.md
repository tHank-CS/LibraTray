...
> **当前状态：**仓库已完成阶段 A/B，并开始阶段 C 的设备适配器与状态协调。
> 现在还没有正式托盘 UI、全局快捷键、Windows 自动化、安装包、签名程序或 GitHub
> Release。固件 38 的一次实机会话已验证发现、状态读取、通知、主灯与氛围灯的
> 基础写入和受限冷启动自愈。设备声明的氛围灯左右分区 RGB 命令虽能即时生效，
> 但它与已观察到的冷启动故障之间尚未完成因果隔离，因此仍未开放。

[English](README.md) | 简体中文

## 截图

正式 UI 属于后续里程碑。UI 真正实现后会用实机截图替换本段；当前不会把效果图冒充为
已实现的软件。
...
这些功能已完成设计，但在当前里程碑**尚未实现**。计划默认快捷键如下：
