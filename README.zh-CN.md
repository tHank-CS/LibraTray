...
> 以及固定全局快捷键均已实现。快捷键配置、设置、Windows 自动化、打包、签名和
> GitHub Release 仍待完成。氛围灯左右分区 RGB 命令虽能即时生效，但它与已观察到的
...
当前快捷键为固定值，并使用 Win32 防重复注册。组合键冲突会显示在面板中，但不会让
应用退出。上一条命令仍在复读确认时，新快捷键会排队并基于最新确认状态执行。每个
Windows 会话只允许运行一个 LibraTray 实例，避免第二个实例把全部组合键误报为占用。
快捷键配置仍在计划中。生命周期自动化将逐项启用并进行防抖。普通界面不会把
`lamp15` 显示成设备名称。
